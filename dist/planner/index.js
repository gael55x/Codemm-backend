...
exports.deriveProblemPlan = deriveProblemPlan;
 ...
 */
function expandDifficultySlots(spec) {
 ...
 */
function distributTopics(spec, slotCount) {
 ...
 */
function deriveProblemPlan(spec) {
    ...
    if (difficulties.length !== spec.problem_count) {
        throw new Error(`Difficulty expansion failed: expected ${spec.problem_count} slots, got ${difficulties.length}.`);
    }
    const topicAssignments = distributTopics(spec, spec.problem_count);
    const slots = difficulties.map((difficulty, index) => ({
        index,
        difficulty,
    ...
    const result = types_1.ProblemPlanSchema.safeParse(slots);
    if (!result.success) {
        const firstError = result.error.issues[0];
        throw new Error(`Invalid ProblemPlan: ${firstError?.message ?? "unknown validation error"}`);
    }
    return result.data;
}

import type { Difficulty } from "../contracts/activitySpec";
...
const COMMON_TOPIC_ALIASES: Record<string, string> = {
  "object oriented": "oop",
  "object-oriented": "oop",
  oop: "oop",
...
export type IntentInterpretation =
  ...
  | { kind: "patch"; patch: JsonPatchOp[]; summaryLines: string[] };

function opForKey(spec: SpecDraft, key: string): "add" | "replace" {
  return (spec as any)[key] == null ? "add" : "replace";
}

function setField(spec: SpecDraft, key: string, value: unknown): JsonPatchOp {
  return { op: opForKey(spec, key), path: `/${key}`, value };
}

function extractLanguage(input: string): "java" | null {
  if (/\bjava\b/i.test(input)) return "java";
  return null;
}

function extractProblemStyle(input: string): "stdout" | "return" | "mixed" | null {
...
function extractProblemCount(input: string): number | null {
  const lower = input.toLowerCase();
...
function extractDifficultyCounts(input: string): DifficultyCounts | null {
  const lower = input.toLowerCase();
...
function toDifficultyPlan(counts: DifficultyCounts) {
...
function normalizeTags(raw: string): string[] {
  ...
  for (const part of parts) {
    ...
    if (part.length <= 40) mapped.push(part);
  }
...
function extractTopicTags(input: string): string[] | null {
  const lower = input.toLowerCase();
  const hasDifficultyWords = /\b(easy|medium|hard)\b/.test(lower);
  if (hasDifficultyWords) return null;
...
export function interpretIntent(spec: SpecDraft, input: string): IntentInterpretation {
  ...
  const inferredCountFromDifficulty =
    difficultyCounts ? difficultyCounts.easy + difficultyCounts.medium + difficultyCounts.hard : null;
  ...
  if (difficultyCounts) {
    ...
    summary.push(
      `difficulty_plan=${plan.map((p) => `${p.difficulty}:${p.count}`).join(", ")}`
    );
  }
  ...
  if (patch.length === 0) return { kind: "none" };

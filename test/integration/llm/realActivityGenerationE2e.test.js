...
test(
  "e2e (real LLM): prompt → dialogue → READY → generateFromSession → activity persisted (providers × 2 × stdout/return/mixed × 4 langs)",
  ...
  async (t) => {
    if (!hasAnyLlmApiKey()) {
      t.skip("No LLM API key set (OpenAI/Anthropic/Gemini).");
      return;
    }
    ...
    const providers = parseCsvEnv("CODEMM_E2E_PROVIDERS", ["openai", "anthropic", "gemini"]);
    const runnableProviders = providers.filter(
      (p) => ["openai", "anthropic", "gemini"].includes(p) && hasProviderKey(p)
    );

    if (runnableProviders.length === 0) {
      t.skip("No runnable providers (check CODEMM_E2E_PROVIDERS + provider API keys).");
      return;
    }
    ...
    try {
      for (const provider of providers) {
        await t.test(`provider=${provider}`, async (tProvider) => {
          if (!["openai", "anthropic", "gemini"].includes(provider)) {
            tProvider.skip(`Unknown provider "${provider}"`);
            summaryRows.push({
              provider,
              language: "",
              style: "",
          ...
          if (!hasProviderKey(provider)) {
            tProvider.skip(`Missing API key for provider=${provider}`);
            summaryRows.push({
              provider,
              language: "",
              style: "",
          ...
          withEnv(tProvider, { CODEX_PROVIDER: provider });
          ...
          for (const language of languages) {
            for (const style of styles) {
              for (const count of counts) {
                ...
                try {
                  await tProvider.test(label, { timeout: 90 * 60 * 1000 }, async () => {
